...
{
    //Retrieves input from remote joystick using getChannel 
    //method through Remote-object 'remote'


    float leftInput = remote.getChannel(Remote::Channel::LEFT_VERTICAL);
    return limitVal(leftInput, -1.0f, 1.0f);
}

float ControlOperatorInterface::getChassisTankRightInput()
{
    float rightInput = remote.getChannel(Remote::Channel::RIGHT_VERTICAL);
    return limitVal(rightInput, -1.0f, 1.0f);
}
}  // namespace control
